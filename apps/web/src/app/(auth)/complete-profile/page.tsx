...
export default function CompleteProfilePage() {
  ...
  const {
    ...
    defaultValues: { employmentType: user?.employmentType || 'SALARIED', annualIncome: '' }
  });

  const onSubmit = async (data: FormValues) => {
    try {
      setServerError('');
      const res = await api.post('/auth/complete-profile', { userId: user?.id, ...data });
      login(res.data.user, res.data.access_token);
      router.push('/dashboard');
    } catch (err: any) {
      setServerError(err.response?.data?.message || 'Profile completion failed.');
    }
  };
  ...
  if (!user) {
    return <p className="text-center mt-20">Please wait...</p>;
  }
    ...
    <motion.div variants={container} initial="hidden" animate="show">
      ...
      <motion.div variants={item} className="card-glass p-7 rounded-2xl">
        <div className="mb-6 text-center">
          <h1 className="text-xl font-bold text-[var(--text-primary)]">Almost there!</h1>
